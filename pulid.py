...
class To_KV(nn.Module):
    def __init__(self, state_dict):
        ...
        for key, value in state_dict.items():
            ...
            self.to_kvs[key.replace(".weight", "").replace(".", "_")].weight.data = (
                value
            )
...
class PulidModelLoader:
    ...
    def load_model(self, pulid_file):
        ...
        if len(st_model["image_proj"].keys()) == 172:
            model = PulidModelV1_1(model)
        elif len(st_model["image_proj"].keys()) == 110:
    ...
    def apply_pulid(
        ...
        for i in range(image.shape[0]):
            ...
            parsing_out = face_helper.face_parse(
                T.functional.normalize(
                    face, [0.485, 0.456, 0.406], [0.229, 0.224, 0.225]
                )
            )[0]
            ...
            for idx in range(len(id_vit_hidden)):
                if noise == 0:
                    ...
                    id_vit_hidden_uncond.append(
                        torch.rand_like(id_vit_hidden[idx]) * noise
                    )
        ...
        if num_zero > 0:
            if noise == 0:
                ...
                zero_tensor = (
                    torch.rand(
                        (cond.size(0), num_zero, cond.size(-1)),
                        dtype=dtype,
                        device=device,
                    )
                    * noise
        ...
        sigma_end = work_model.get_model_object("model_sampling").percent_to_sigma(
            end_at
        )
